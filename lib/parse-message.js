...
exports.directMessage = function (message, socket, theOnlineUsersArray) {
  ...
  theOnlineUsersArray.forEach( s => {
    if (s.username === toUserName )
      s.write(`${socket.username}: ${messageContent}`);
  });//end of loop
...
exports.banUser = function (message, socket, theOnlineUsersArray){
  let toUserName = message.split(' ')[1].trim();
  theOnlineUsersArray.forEach(  (s, index) =>  {
    if (s.username === toUserName )
      theOnlineUsersArray.splice(index, 1);
    s.end (`${socket.username} has been banned from Kens Chatroom`);
  });
